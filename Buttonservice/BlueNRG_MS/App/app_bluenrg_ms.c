...
static void User_Process(void)
{
  ...
  Timer_STATE++;
  /* Check if the timer has been hit */
  if (Timer_STATE>250)
  {
	  Timer_STATE=0;
    /* Debouncing */
...
//    if (connected && notification_enabled)
    if (connected)
    {
      ...
      /* Send a toggle command to the remote device */
    	uint8_t data_on[20] = {'B','u','t','t','o','n',' ','S','t','a','t','e',' ','i','s',' ','o','n',' ',' '};
    	    	uint8_t data_off[20] = {'B','u','t','t','o','n',' ','S','t','a','t','e',' ','i','s',' ','o','f','f',' '};


      if (BSP_PB_GetState(BUTTON_KEY))
    	  sendData(data_on, sizeof(data_on));
      else
    	  sendData(data_off, sizeof(data_off));
